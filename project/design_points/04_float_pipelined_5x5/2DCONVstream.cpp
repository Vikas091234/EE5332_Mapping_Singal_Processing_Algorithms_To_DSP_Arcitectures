...
#pragma HLS ARRAY_PARTITION variable=local_kernel complete dim=1
    localKernel:for (int i = 0; i < K*K; i++) {
...
#pragma HLS ARRAY_PARTITION variable=window complete dim=0
    ...
    // Zero everything
    linebuf_init:for (int j = 0; j < MAX_WIDTH + 2*PAD; j++) {
//#pragma HLS PIPELINE II=1
        line_buf[0][j] = 0.0f;
        line_buf[1][j] = 0.0f;
        line_buf[2][j] = 0.0f;
    ...
    // Ã¢â€?â‚¬Ã¢â€?â‚¬ Iterate over PADDED image: (rows+2*PAD) x (cols+2*PAD) Ã¢â€?â‚¬Ã¢â€?â‚¬
...
#pragma HLS LOOP_TRIPCOUNT min=66 max=66 avg=66
        mainLoop_cols:for (int pj = 0; pj < padded_cols; pj++) {
#pragma HLS LOOP_TRIPCOUNT min=66 max=66 avg=66
#pragma HLS PIPELINE II=1

            // Ã¢â€?â‚¬Ã¢â€?â‚¬ Determine pixel value (real or zero pad) Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬Ã¢â€?â‚¬
            ...
            // out-of-bounds Ã¢â€ â€™ new_pixel stays 0.0f (matches fillvalue=0)
            ...
            window[2][K-1] = line_buf[2][pj];
            window[3][K-1] = line_buf[3][pj];
            window[4][K-1] = new_pixel;

            line_buf[0][pj] = line_buf[1][pj];
            line_buf[1][pj] = line_buf[2][pj];
            line_buf[2][pj] = line_buf[3][pj];
            line_buf[3][pj] = new_pixel;
            ...
            for (int r = 0; r < K; r++) {
//#pragma HLS UNROLL
                acc_innerLoop:for (int c = 0; c < K; c++) {
//#pragma HLS UNROLL
                    acc += window[r][c] * local_kernel[r * K + c];
            }}

            // Ã¢â€?â‚¬Ã¢â€?â‚¬ Write output only for real (unpadded) pixel positions
            // The window at padded position (pi,pj) is centered on
            // (pi-PAD, pj-PAD) in the original image Ã¢â‚¬â€? so output is
