...
entity conv2d_optimized_control_s_axi is
generic (
...
end entity conv2d_optimized_control_s_axi;
...
-- 0x1c : reserved
-- 0x20 ~
-- 0x3f : Memory 'kernel' (9 * 16b)
--        Word n : bit [15: 0] - kernel[2n]
--                 bit [31:16] - kernel[2n+1]
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of conv2d_optimized_control_s_axi is
...
attribute DowngradeIPIdentifiedWarnings of behave : architecture is "yes";
    ...
    signal kernel_shift0       : UNSIGNED(0 downto 0);
    signal int_kernel_shift0   : UNSIGNED(0 downto 0);

    component conv2d_optimized_control_s_axi_ram is
        generic (
    ...
    end component conv2d_optimized_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_kernel
int_kernel : conv2d_optimized_control_s_axi_ram
generic map (
     MEM_STYLE  => "auto",
     MEM_TYPE   => "2P",
     BYTE_WIDTH => 8,
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    rdata_data <= RESIZE(int_kernel_q1, 32);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- kernel
    int_kernel_address0  <= RESIZE(SHIFT_RIGHT(UNSIGNED(kernel_address0), 1), 3);
    int_kernel_ce0       <= kernel_ce0;
    kernel_q0            <= STD_LOGIC_VECTOR(RESIZE(SHIFT_RIGHT(int_kernel_q0, TO_INTEGER(int_kernel_shift0) * 16), 16));
    ...
    int_kernel_we1       <= '1' when int_kernel_write = '1' and w_hs = '1' else '0';
    int_kernel_be1       <= UNSIGNED(WSTRB) when int_kernel_we1 = '1' else (others=>'0');
    int_kernel_d1        <= UNSIGNED(WDATA);
    kernel_shift0        <= RESIZE(UNSIGNED(kernel_address0(0 downto 0)), 1);
...
entity conv2d_optimized_control_s_axi_ram is
    generic (
        MEM_STYLE  : STRING :="auto";
        MEM_TYPE   : STRING :="S2P";
        BYTE_WIDTH : INTEGER :=8;
...
end entity conv2d_optimized_control_s_axi_ram;

architecture behave of conv2d_optimized_control_s_axi_ram is
